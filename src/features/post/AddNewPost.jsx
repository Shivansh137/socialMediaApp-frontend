...
const AddNewPost = () => {
    ...
    return (
        <Main className={'relative'}>
            ...
            <input ref={inputRef} onChange={handleFiles} type="file" multiple={true} name="file" id="" hidden={true} />
            <input ref={addMoreInputRef} onChange={handleFiles} type="file" multiple={true} name="addfile" id="" hidden={true} />
            {
           ...
           <section className="flex p-4 rounded-md">
                  <button onClick={()=>{setRatio('1/1')}}  className={` w-10 h-8 rounded-l-md transition-[background-color] delay-50 text-xs ${ratio === '1/1' ? 'bg-blue-500 text-white':' bg-gray-100 dark:bg-dark-sec'}`} type="button">
                    1 : 1
                  </button>
                  <button onClick={()=>{setRatio('4/5')}} className={` w-10 h-8  rounded-r-md transition-[background-color] delay-50 text-xs  ${ratio !== '1/1' ? 'bg-blue-500 text-white':'bg-gray-100 dark:bg-dark-sec'}`} type="button">
                    4 : 5
                  </button>
                  <button onClick={()=>{navigate('/')}} className="ml-auto text-sm">Cancel</button>
                </section>
           
                    <ul className={` py-2 rounded-lg overflow-y-hidden space-x-2 whitespace-nowrap`}>
                   {
               ...
               <button onClick={()=>{addMoreInputRef.current?.click()}} className=" py-2 text-sm bg-dark-sec rounded-full w-full my-2">Add More + </button>
                <form className="flex flex-col gap-3 my-4">
                    ...
                    <button type="button" onClick={handleSubmit} className="px-4 py-2 text-sm bg-sky-500 dark:bg-blue-600 text-white rounded-lg w-full">Create New Post</button>
                </form>
                </> : ''
            }
        </Main>
