  ...
  if (isSuccess && isSuccessPosts) return (
    <>
      ...
      <Main className={'px-4 md:p-8'}>
       ...
       {
            user?.profilePic ? <div onClick={() => { setShowPic(true) }} className='w-24 h-24 rounded-full my-4 dark:bg-dark-sec bg-cover bg-center bg-no-repeat bg-[rgba(10,10,10,1)]' style={{ backgroundImage: `url(https://res.cloudinary.com/dofd4iarg/image/upload/v1690608655/${user?.profilePic}.png)` }}></div> : <p><MdAccountCircle className='text-slate-300 text-6xl' /></p>
          }
         {
        ...
        <li className='w-full'>
            <button onClick={() => { postsRef.current?.scrollIntoView({ behavior: 'smooth' }) }} className='flex flex-col shadow-md dark:bg-dark-sec w-full rounded-lg p-4 items-center'>
              <p className='text-2xl'>{posts?.length}</p>
              <p className='text-[calc(.5rem+2px)]'>Posts</p>
            </button>
          </li>
          <li className='w-full'>
            <Link to={'followers'} className='flex flex-col shadow-md dark:bg-dark-sec w-full rounded-lg p-4 items-center'>
              <p className='text-2xl'>{user?.followers?.length}</p>
              <p className='text-[calc(.5rem+2px)]'>Followers</p>
            </Link>
          </li>
          <li className='w-full'>
            <Link to={'following'} className='flex flex-col shadow-md dark:bg-dark-sec w-full  rounded-lg p-4 items-center'>
        ...
        <section className='flex md:px-4 gap-2 pb-4'>
          <FollowButton username={username} classname='text-sm w-full' />
          <button className='dark:bg-gray-700 dark:text-neutral-300 bg-neutral-300 text-neutral-500 px-4 py-2 text-sm rounded-md w-full'>
